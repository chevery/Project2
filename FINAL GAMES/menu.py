...
ABOUT = ['{0}'.format(pygameMenu.__version__),
         '{0}'.format(pygameMenu.__author__),
         '{0}'.format(pygameMenu.__email__)]
...
def play_function(difficulty, font):
    ...
    if difficulty == 'EASY':
        import level1_easy.py
    elif difficulty == 'MEDIUM':
        f = font.render('Playing level 1, Collect The Apples', 1, COLOR_WHITE)
        import level1_medium.py
    elif difficulty == 'HARD':
        f = font.render('Playing level 1, Collect The Apples', 1, COLOR_WHITE)
        import level1_hard.py
    else:
...
def main_background():
    ...
    START_IMG = 'MainMenupic2.png'
    start_img = pygame.image.load(path.join(img_folder, START_IMG)).convert_alpha()
